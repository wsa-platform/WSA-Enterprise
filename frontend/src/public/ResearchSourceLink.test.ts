...
describe('research source title destination', () => {
  ...
  it('C: Library file identifiers are not a scientific source destination', () => {
    const citation = {
      citation_id: 'file-1',
    ...
    expect(html).toContain('Preserved paper')
    expect(html).not.toContain('<a')
    expect(html).not.toContain('/api/v1/library/files/')
    expect(html).not.toContain('/research/result/')
  })

  it('D: no URL or DOI — title is not a link and does not open the Viewer', () => {
