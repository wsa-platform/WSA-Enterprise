...
type ServicesPortalNavDropdownProps = {
  onNavigate?: () => void
}

/** Header navigation dropdown for بوابة الخدمات. */
export function ServicesPortalNavDropdown({ onNavigate }: ServicesPortalNavDropdownProps) {
  ...
  const active = isServicesPortalPath(pathname)
  ...
  return (
    ...
    >
      ...
      <div
        ...
        role="menu"
        aria-label={t('website.nav.servicesPortal')}
        hidden={!open}
      >
        <ul className="gs-nav-dropdown-list">
          {SERVICES_PORTAL_MENU_ITEMS.map((item) => (
